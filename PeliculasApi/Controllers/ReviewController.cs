...
{
    [Microsoft.AspNetCore.Mvc.Route("api/peliculas/{peliculaId:int}/reviews")]
    ...
    public class ReviewController : CustomBaseController
    {
        ...
        [HttpPut("{reviewId}")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
        public async Task<ActionResult> Put(int reviewId, int peliculaId, [FromBody] ReviewCreacionDTO reviewCreacionDTO)
        {
            ...
            if (reviewDB.UsuarioId != usuarioId) { return BadRequest("No tienes permisos para editar este review"); }
